...
class GetNotesFromMailIDUseCase(
    private val repository: NoteRepository
) {

    operator fun invoke(
        noteOrder: NoteOrder = NoteOrder.Date(orderType = OrderType.Descending),mailId: String
    ): Flow<List<Note>> {
        return repository.getNoteByMail(mailId = mailId).map { notes ->
